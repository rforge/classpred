%\VignetteIndexEntry{OOMPA ClassPrediction}
%\VignetteKeywords{OOMPA,Class Prediction,Genetic Algorithm}
%\VignetteDepends{oompaBase,PreProcess,ClassPrediction}
%\VignettePackage{ClassPrediction}
...
\item model optimzation: selecting parameters to combine the selected
...
<<lib,eval=FALSE>>=
library(ClassPrediction)
@ 
<<removeMe,echo=FALSE>>=
source("../../R/cp01-genalg.R")
@ 
...
The \Rclass{genalg} class in the \Rpackage{ClassPrediction} library
...
algorithm is intialized by creating a \textit{population} of such
...
web site at \url{http://www.versus.com/tdfgames}.  The (simplified)
...
@ 
<<removeMe,echo=FALSE>>=
load("../../data/gaTourResults.rda")
@ 

In Figure~\ref{gens}, we plot the best fitness and the mean fitness as
a function of the number of genrations through which the genetic
...
change can improve the score.  We also consider the possibilty of
...
tour, three riders wore the ``yellow jersey'' for the overall leade:
...
classification'' for overlall best time.  Riders who win breakaways
...
themsleves, gives a reliable measure of the convergence of the genetic
...
the gain in finding the optimal solution occured in the first $300$ to
$400$ generations.
<<loess>>=
...
\caption{Relationship between the mean and maximum fitness in a
  population of potential solutions as the geneerations evolve.  Gray
...
\Rpackage{ClassPrediction} package; we used this function above when
...
par(opar)
@ 
\caption{Overaly of the average diversity (color) on plots of the best
  (black) and mean (gray) fitness through the generations of the
  genetic algorithm.  Horizontal (cyan) lines are locatyed at a
...
genration zero at a value between eight and nine.  This observation
...
Howevre, the diversity is near $1.5$ during the period when the best
...
convergence.  An additonal measure that might be applied when the
...
sum(temp)
@ 
In this case, rhe best solution is represented by \Sexpr{sum(temp)}
...
to the problem of convergence.  The cycling example is likley to be
easier than the gen epxression problem, since there are only $102$
...
from a list of $102$.)  Unless some preliminary filtering ir performed
...
useful predicitve features.

We do expect, however, that the ``diversity'' wil prove useful to
monitor convergence even in the gene expression setting.  As mentioned
earlier, diversity has the advantage fo providing an interpretable
condition for converence, which doe s not need to know the ``fitness''
of the optimal solution in advance.  By contrast, we know that the
fitness function must will be different in the gene epxression case
...
\Rpackage{ClassPrediction} package.  A fitness function that is
